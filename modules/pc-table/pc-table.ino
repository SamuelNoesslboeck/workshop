...
void loop() {
  ...
  if (ut.check(up.check(pc_table::upper_encoder.check_switch()))) {
    ...
    mqtt_client.publish(TOPIC_LIGHT_CHAIN_MAIN, (const uint8_t*)&TRUE, (unsigned int)1, true);
  }

  if (lt.check(lp.check(pc_table::lower_encoder.check_switch()))) {
    debugln("| > Lower switch pressed!");
    mqtt_client.publish(TOPIC_LIGHT_CHAIN_MAIN, (const uint8_t*)&FALSE, (unsigned int)1, true);
  }
